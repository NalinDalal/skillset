...
def unquote(text):
    ...
    out = []
    for line in text.split("\n"):
        m = re.match(r'^(\s*[a-z_-]+:\s*)"(.*)"\s*$', line, re.S)
        if m:
        ...
        for seg in re.split(r'(")', line):
            if seg == '"':
                in_q = not in_q
                continue
            res.append(" " * len(seg) if in_q else seg)
        out.append("".join(res))
    return "\n".join(out)

def lint(text, strict=False):
    raw = text
    text = unquote(strip_code(text))
    ...
    v["contraction"] = len(re.findall(
        r"\b(?:\w+['’](?:t|re|ve|ll|d|m)\b|(?:it|that|who|what|there|here|he|she|let|how|where|when|why)['’]s\b)",
        text))
    ...
    def is_list_block(p):
        ...
        listy = [l for l in lines if re.match(r"^\s*(?:[-*+]|\d+[.)])\s+", l)]
        return len(listy) / len(lines) >= 0.5
    def is_description(p):
        first = next((l for l in p.split("\n") if l.strip()), "")
        return bool(re.match(r"^\s*(?:description|motion_description|additional-description):", first))
    v["long_paragraph(>6s)"] = sum(1 for p in paras if not is_description(p) and not is_list_block(p) and len(sentences(strip_code(p))) > 6)
