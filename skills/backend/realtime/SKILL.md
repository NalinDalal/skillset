...
export class ConnectionHandler {
  ...
  onOpen(ws: WebSocket) {
    ...
    // Send welcome
    ws.send(
      JSON.stringify({ type: "CONNECTED", payload: { userId: user.id } }),
    );
...
export type ClientMessage =
  ...
  | {
      type: "CURSOR_UPDATE";
      payload: { roomId: string; position: CursorPosition };
    }
...
export class RoomManager {
  ...
  leaveRoom(roomId: string, userId: string) {
    ...
    this.broadcast(
      roomId,
      { type: "MEMBER_LEFT", payload: { userId, roomId } },
      userId,
    );
...
export async function authMiddleware(
  req: Request,
  server: Server,
  next: (user: User, token: string) => Response,
) {
...
export function useRealtime(roomId: string) {
  ...
  useEffect(() => {
    ...
    ws.current.onopen = () => {
      setConnected(true);
      ws.current!.send(
        JSON.stringify({ type: "JOIN_ROOM", payload: { roomId } }),
      );
    };
  ...
  const handleMessage = (message: ServerMessage) => {
    ...
    switch (message.type) {
      ...
      case "MEMBER_LEFT":
        setMembers((prev) =>
          prev.filter((m) => m.userId !== message.payload.userId),
        );
        break;
      case "CURSOR_UPDATE":
        setCursors((prev) =>
          new Map(prev).set(message.payload.userId, message.payload.position),
        );
        break;
    }
  };
