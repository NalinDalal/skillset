...
export async function createAccessToken(
  payload: Omit<AccessPayload, "type">,
): Promise<string> {
...
- `backend/database` - Session storage, audit logs
- `backend/realtime` - WS authentication
- `backend/api-design` - Auth middleware patterns
- `devops/testing` - Testing auth flows
